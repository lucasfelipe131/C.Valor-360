...
-- conteúdo da fonte.
-- Uma coluna por ALTER: o contrato de schema da CI (scripts/lib/schema-contract.mjs) lê um ADD
-- COLUMN por instrução, e um ALTER com três colunas deixava duas fora do contrato e derrubava o
-- db:drift --strict.
ALTER TABLE val_knowledge_source_requests ADD COLUMN IF NOT EXISTS candidates JSONB NOT NULL DEFAULT '[]'::jsonb;
ALTER TABLE val_knowledge_source_requests ADD COLUMN IF NOT EXISTS candidates_researched_at TIMESTAMPTZ;
ALTER TABLE val_knowledge_source_requests ADD COLUMN IF NOT EXISTS candidates_researched_by VARCHAR(320);
