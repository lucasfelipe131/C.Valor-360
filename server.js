...
const mime={'.geojson':'application/geo+json','.html':'text/html; charset=utf-8','.css':'text/css; charset=utf-8','.js':'application/javascript; charset=utf-8','.svg':'image/svg+xml','.json':'application/json; charset=utf-8','.png':'image/png','.jpg':'image/jpeg','.jpeg':'image/jpeg','.ico':'image/x-icon','.webp':'image/webp','.woff2':'font/woff2'}
const securityHeaders={'X-Content-Type-Options':'nosniff','Referrer-Policy':'strict-origin-when-cross-origin','Permissions-Policy':'camera=(self), microphone=(self), geolocation=(self)','Content-Security-Policy':"default-src 'self'; script-src 'self'; style-src 'self' 'unsafe-inline'; img-src 'self' data: blob: https://server.arcgisonline.com https://tile.openstreetmap.org; connect-src 'self' https://api.openai.com wss://api.openai.com; font-src 'self'; worker-src 'self' blob:; media-src 'self' blob: data:; frame-src 'self'; object-src 'none'; base-uri 'self'; frame-ancestors 'none'; form-action 'self'"}
...
async function handleApi(request,response,url){
 ...
 // O Estudio de Conversao buscava /api/clients/:id/context, que devolve o COMPLEMENTO TECNICO
 // (getTechnicalContext). Ele nunca teve a chave conversionInnovations, montada so por
 // innovation-bootstrap.js sobre ValRepository.getClientContext — e nenhuma rota HTTP expunha esse
 // metodo. Resultado: os seis paineis recebiam undefined, o de expansao pos-conversao sumia e os
 // outros escreviam "ainda nao registrado" para um produtor com oportunidade registrada e negocios
 // ganhos. Rota propria, ao lado da existente: a de Cliente 360 continua barata, e so quem abre o
 // Estudio paga o contexto completo.
 const studioMatch=url.pathname.match(/^\/api\/clients\/([^/]+)\/conversion-studio$/)
 if(studioMatch&&request.method==='GET'){
  const clientId=decodeURIComponent(studioMatch[1])
  const context=await repository.getClientContext({tenantId:identity?.tenantId||config.defaultTenantId,ownerId:identity?.id||identity?.email,clientId,
   contextRequest:{requestId:requestIdFrom(request),tenantId:identity?.tenantId||config.defaultTenantId,ownerId:identity?.id||identity?.email,producerId:clientId,objective:'conversion_studio',
    actorRole:identity?.role||'consultant',scope:'own_portfolio',databaseTimeoutMs:config.databaseQueryTimeoutMs}})
  if(!context?.client?.id)return json(response,404,{error:'Produtor não encontrado na sua carteira autorizada.'})
  return json(response,200,{conversionInnovations:context.conversionInnovations||{},opportunities:context.opportunities||[],
   client:{id:String(context.client.id),name:context.client.name||''},generatedAt:new Date().toISOString()})
 }
 ...
 const seasonPlanMatch=url.pathname.match(/^\/api\/clients\/([^/]+)\/season-plans$/)
 if(seasonPlanMatch&&request.method==='GET')return json(response,200,await readProducerSeasons(repository,decodeURIComponent(seasonPlanMatch[1]),identity?.id||identity?.email))
 if(seasonPlanMatch&&request.method==='PUT'){
  const clientId=decodeURIComponent(seasonPlanMatch[1]);const season=await saveProducerSeason(repository,clientId,identity?.id||identity?.email,await body(request))
  return json(response,200,{season})
 }
 const workspaceMatch=url.pathname.match(/^\/api\/clients\/([^/]+)\/workspace$/)
 if(workspaceMatch&&request.method==='GET')return json(response,200,await readProducerWorkspace(repository,decodeURIComponent(workspaceMatch[1]),identity?.id||identity?.email))
 const propertyMatch=url.pathname.match(/^\/api\/clients\/([^/]+)\/property$/)
 if(propertyMatch&&request.method==='GET')return json(response,200,await repository.getPropertyProfile(decodeURIComponent(propertyMatch[1]),identity?.id||identity?.email,{propertyId:url.searchParams.has('propertyId')?url.searchParams.get('propertyId'):undefined}))
 if(propertyMatch&&request.method==='PUT'){
  const clientId=decodeURIComponent(propertyMatch[1]);const profile=await repository.savePropertyProfile(clientId,await body(request),identity?.id||identity?.email);invalidateValContextScope({tenantId:identity?.tenantId||config.defaultTenantId,ownerId:identity?.id||identity?.email,clientId});await accessRepository.recordUsage(identity,{eventType:'property_saved',page:'client360',entityType:'client',entityId:clientId,metadata:{fields:profile.fields.length,located:Boolean(profile.property?.location)}});return json(response,200,profile)
 }
