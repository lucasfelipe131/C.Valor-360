...
export default function PublicSurvey({token}){
 ...
 if(screen==='form')return <div className="public-survey-shell form-mode"><header className="public-survey-header"><Logo/><span><LockKeyhole size={13}/>Conexão segura</span></header><SurveyForm producerName={invitation?.producerName} onSubmit={submit}/><footer className="public-survey-footer">VALOR 360 • Conhecer para gerar valor</footer></div>
 return <div className="public-survey-shell welcome-mode"><div className="survey-aurora one"/><div className="survey-aurora two"/><section className="public-welcome"><Logo/><div className="welcome-orb"><Sparkles/></div><span className="welcome-kicker">PRODUTOR 360 • VALOR 360</span><h1>Queremos conhecer o contexto e as preferências da sua propriedade.</h1><p>Suas respostas ajudam a preparar um atendimento mais próximo e alinhado ao que gera valor para você.</p><div className="welcome-facts"><span><Clock3/>5 a 7 minutos</span><span><LockKeyhole/>Uso no relacionamento comercial</span></div>{invitation?.producerName&&<div className="welcome-name">Convite preparado para <b>{invitation.producerName}</b></div>}<button onClick={()=>setScreen('form')}>Começar agora <ArrowRight size={19}/></button><small>Ao continuar, você confirma que leu esta finalidade. Você pode pedir correção ou exclusão das respostas à equipe responsável. Não existem respostas certas ou erradas.</small></section></div>
}
