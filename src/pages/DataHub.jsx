...
export default function DataHub({clients=[],onImport,onProfileImport,onUpdate,onDelete,onNotify}){
 ...
 return <div className="page-stack data-hub-page">
  ...
  {stage==='done'&&<section className="learning-result"><div className="result-glow"><DatabaseZap/></div><span className="eyebrow">BASE INCORPORADA</span><h2>O contexto comercial foi atualizado.</h2><p>Os fatos históricos foram incorporados ao Cliente 360; sinais e hipóteses continuam sujeitos à validação do consultor.</p><div className="learning-metrics"><div><small>PRODUTORES</small><b>{result.clientCount}</b></div><div><small>REGISTROS INCORPORADOS</small><b>{result.rowsIncorporated??result.rowCount}{result.truncated&&result.rawRowCount?` de ${result.rawRowCount}`:''}</b></div><div><small>ÍNDICE ALTO</small><b>{result.highIndex??0}</b></div><div><small>VOLUME INFORMADO</small><b>R$ {(result.totalRevenue/1000).toFixed(0)} mil</b></div></div>
   {(result.truncated||result.clientsTruncated||result.archivedSkipped?.length)&&<div className="form-error" role="alert"><b>Nem tudo da planilha entrou na base.</b><ul>
