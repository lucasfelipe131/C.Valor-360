...
const Drilldown=({eyebrow,title,children})=><details className="client-drilldown" open><summary><span><small>{eyebrow}</small><b>{title}</b></span><ChevronDown/></summary><div className="client-drilldown-content">{children}</div></details>
...
const money=value=>value===null||value===undefined||value===''?'Não informado':`R$ ${Number(value).toLocaleString('pt-BR',{maximumFractionDigits:2})}`
...
export default function Client360Details({section,initialPropertyId,onPropertyChange,client,visits=[],opportunities=[],storageScope,onBack,onPrepare,onUpdate,onSaved,onRefreshPortfolio,onAsk}){
 ...
 // O `error` da linha de baixo e compartilhado com o salvamento e e zerado no clique, entao nao
 // serve para saber se a LEITURA do complemento deu certo. Sem esse desfecho proprio, a tela
 // nao distingue "nao existe memoria" de "nao consegui ler a memoria" - e o PUT e substituicao
 // total: salvar com a tela vazia apaga daninhas, doencas, solo, meta e concorrentes que
 // estavam gravados.
 const [contextRead,setContextRead]=useState('loading')
 ...
 useEffect(()=>{
  const controller=new AbortController();const started={...revisions.current};setLoadingContext(true);setContextRead('loading');setError('')
  ...
  fetch(`/api/clients/${encodeURIComponent(client.id)}/context`,{signal}).then(async response=>{if(response.status===401){window.dispatchEvent(new Event('valor360:unauthorized'));throw new Error('Sua sessão expirou.')}if(!response.ok)throw new Error('O complemento salvo não pôde ser carregado.');return response.json()}).then(payload=>{if(payload.context){mergeRemote(payload.context,started);setContextMeta({status:payload.context.status||'',updatedAt:payload.context.updatedAt||''})}setContextRead('ok')}).catch(exception=>{if(exception.name!=='AbortError'){setContextRead('failed');setError(exception.name==='TimeoutError'?'O servidor demorou para carregar o complemento.':exception.message)}}).finally(()=>{if(!controller.signal.aborted)setLoadingContext(false)})
