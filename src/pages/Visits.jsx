...
export default function Visits({clients,visits,onSave,onPrepare}){
 ...
 const ordered=useMemo(()=>[...visits].sort((a,b)=>visitDate(a)-visitDate(b)),[visits])
 const save=async e=>{
  e.preventDefault();if(!form.clientId||!form.date||!form.objective)return
  setSaving(true);setError('')
  try{await onSave?.({clientId:form.clientId,scheduledAt:new Date(`${form.date}T${form.time}:00`).toISOString(),objective:form.objective});setShowForm(false);setForm({...form,objective:''})}catch(exception){setError(exception.message||'Não foi possível salvar a visita.')}finally{setSaving(false)}
 }
