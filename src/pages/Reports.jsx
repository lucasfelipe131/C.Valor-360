...
export default function Reports({clients,visits}){
 ...
 const potential=clients.reduce((sum,c)=>sum+Number(c.commercial?.potential||0),0)
 ...
 return <div className="page-stack report-page">
  ...
  <section className="kpi-grid"><KpiCard icon={Users} label="Produtores mapeados" value={clients.length} delta="Base piloto"/><KpiCard icon={HeartHandshake} label="IRT médio" value={irt} delta="Relacionamento estratégico" tone="cyan"/><KpiCard icon={Percent} label="Promotores NPS" value={promoters} delta={`${Math.round(promoters/Math.max(clients.length,1)*100)}% da carteira`} tone="green"/><KpiCard icon={Target} label="Potencial identificado" value={`R$ ${Math.round(potential/1000)} mil`} delta="Oportunidades abertas"/></section>
  <section className="report-grid"><article className="panel"><div className="panel-head"><div><span className="eyebrow">PERFIL DA CARTEIRA</span><h3>DNA comercial dos produtores</h3></div><FileBarChart/></div><div className="profile-bars">{profiles.map((profile,i)=><div key={profile}><span>{profile}</span><div><i style={{width:`${counts[i]/max*100}%`}}></i></div><b>{counts[i]}</b></div>)}</div></article><article className="panel pilot-score"><span className="eyebrow">PRONTIDÃO DO MVP</span><div className="score-ring"><div><b>72%</b><span>piloto</span></div></div><ul><li><i></i>Cliente 360 e perfis ativos</li><li><i></i>VAL demonstrável</li><li><i></i>Agenda e oportunidades navegáveis</li><li className="pending"><i></i>API e autenticação corporativa</li></ul></article></section>
