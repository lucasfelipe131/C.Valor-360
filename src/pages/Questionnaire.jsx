...
export default function Questionnaire({onCreate,onOpen,onNotify}){
 ...
 const createInvitation=async()=>{
  ...
  try{
   ...
   const response=await fetch('/api/surveys/invitations',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({token,producerName:producerName.trim(),consultantName:'Lucas Felipe'})})
 ...
 return <div className="page-stack producer-lab">
  ...
  {mode==='central'&&<>
   ...
   <section className="invite-composer"><div className="invite-copy"><span className="eyebrow">NOVO CONVITE</span><h2>Crie uma experiência individual.</h2><p>O nome é opcional. Cada produtor recebe um link único, pensado para responder pelo celular.</p></div><div className="invite-form"><label>Nome do produtor <span>opcional</span><input value={producerName} onChange={event=>setProducerName(event.target.value)} placeholder="Ex.: João da Silva"/></label><button disabled={loading} onClick={createInvitation}><Plus/>{loading?'Criando...':'Gerar link do questionário'}</button></div></section>
