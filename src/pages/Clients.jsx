...
export default function Clients({clients=[],opportunities=[],onClient,onNew}){
 ...
 return <div className="page-stack producer-list-page">
  ...
  <div className="list-summary"><span>Exibindo <b>{Math.min(visibleCount,list.length)}</b> de {list.length} encontrados • {clients.length} na carteira</span><span>Dados protegidos no seu login</span></div>
  {list.length?<><section className="client-grid">{visibleList.map(item=>{const c=item.client;const metrics=item.metrics;const opportunity=item.opportunity;const days=lastContact(c);const opportunityTitle=opportunity?.title||item.candidate?.title||c.commercial?.opportunity||'Descoberta ainda não iniciada';return <article className="client-card" key={c.id} role="button" tabIndex="0" aria-label={`Abrir visão 360 de ${c.name}`} onClick={()=>openClient(item)} onKeyDown={event=>{if(event.key==='Enter'||event.key===' '){event.preventDefault();openClient(item)}}}>
  ...
  </article>})}</section>{visibleCount<list.length&&<button type="button" className="producer-list-more" onClick={()=>setVisibleCount(count=>count+9)}>Mostrar mais produtores <ChevronRight/></button>}</>:<section className="producer-list-empty"><Search/><h3>Nenhum produtor encontrado</h3><p>Ajuste a busca ou os filtros; nenhum cadastro foi alterado.</p><button type="button" onClick={()=>{setQ('');setFilter('all')}}>Limpar filtros</button></section>}
 </div>
}
