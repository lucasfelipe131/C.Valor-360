...
export function buildPrepareVisitPresentation({prepared={},client={},visit={},preference='SIMPLE'}={}){
 ...
 const probableObjection=/nenhuma objeção material/i.test(clean(preparation.probable_objection))?'':clean(preparation.probable_objection)
 const littleHistory=preparation.quality_audit?.status==='INSUFFICIENT_CONTEXT'||missing.length>0&&!probableObjection&&!clean(preparation.main_opportunity)
 ...
 const attention=compact([
  ...(Array.isArray(preparation.material_attention)?preparation.material_attention:[]),
 ...
 const whyNow=first(preparation.why_now)
 const avoid=first(preparation.avoid_guidance,preparation.objection_guidance,'Não avance para proposta antes de confirmar o que pesa na decisão.')
 const proofs=compact(preparation.proofs_to_take,3)
 ...
 return {
  ...
  essential:{objective,whyNow,attention,questions,strategy,avoid,proofs,commitment},
  quick:{
   situation:first(preparation.why_now,attention[0],objective),
   opportunity:first(preparation.main_opportunity,objective),
   risk:first(probableObjection,attention[0],missing[0],'Nenhum risco crítico confirmado.'),
   avoid,
   questions,
   commitment
  },
