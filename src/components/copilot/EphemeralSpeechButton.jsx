...
export default function EphemeralSpeechButton({disabled=false,onTranscript,onError,autoStartKey='',onListeningChange}){
 const [listening,setListening]=useState(false)
 const recognitionRef=useRef(null)
 const lastAutoStartKeyRef=useRef('')
 useEffect(()=>()=>{try{recognitionRef.current?.abort()}catch{}},[])
 const toggle=()=>{
  if(listening){try{recognitionRef.current?.stop()}catch{};setListening(false);onListeningChange?.(false);return}
  ...
  recognition.onstart=()=>{setListening(true);onListeningChange?.(true)}
  recognition.onend=()=>{setListening(false);onListeningChange?.(false)}
  recognition.onerror=event=>{setListening(false);onListeningChange?.(false);onError?.(event?.error==='not-allowed'?'Permita o uso do microfone para falar com a VAL.':'Não consegui ouvir com clareza. Tente novamente.')}
 ...
 useEffect(()=>{
  const key=String(autoStartKey||'')
  if(!key||disabled||lastAutoStartKeyRef.current===key)return
