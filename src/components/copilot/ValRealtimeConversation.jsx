...
export function ValRealtimeConversationStage({
 ...
 // "Continua liberado" só é verdade quando o microfone foi obtido nesta tentativa. Falha antes do
 // getUserMedia (status, sessão, contexto da conversa) não sabe nada sobre ele e volta ao texto neutro.
 const transportFault=!microphoneFault&&state.microphonePermission==='GRANTED'
 ...
 return <section className={rootClass} aria-label="Modo conversa por voz" data-version={state.version||REALTIME_CONVERSATION_POLICY.version} data-transport={state.transport||'WEBRTC'} data-microphone-active={state.microphoneActive?'true':'false'}>
  <div className="val-conversation-topline">
   ...
   <span className={`val-conversation-connection ${state.microphoneActive?'is-live':''}`}><span aria-hidden="true"/>{connecting?'Conectando':state.microphoneActive?'Microfone ativo':unavailable&&transportFault?'Voz desconectada':'Microfone desligado'}</span>
  </div>
