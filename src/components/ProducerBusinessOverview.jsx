...
const optional=value=>known(value)?Math.max(0,Number(value)):null
const money=value=>!known(value)?'—':Number(value).toLocaleString('pt-BR',{style:'currency',currency:'BRL',maximumFractionDigits:2})
...
const countLabel=(value,singular,plural)=>!known(value)?'Não informado':`${finite(value).toLocaleString('pt-BR')} ${finite(value)===1?singular:plural}`
...
function Metric({icon:Icon,label,value,detail,tone='',loading=false}){
 return <article className={`business-metric ${tone}`}><span><Icon/></span><div><small>{label}</small><b>{loading?<span className="p360-skeleton" role="status" aria-label="Carregando dado"/>:value}</b><em>{detail}</em></div></article>
}
...
export default function ProducerBusinessOverview({client,refreshToken=0}){
 ...
 const fallback=useMemo(()=>{
  ...
  const current=optional(c.purchaseCurrentSeason),potential=optional(c.potentialTotal)
  const open=known(c.openPotential)?optional(c.openPotential):current!==null&&potential!==null?Math.max(0,potential-current):null
  const creditLimit=optional(c.creditLimit),creditUsed=optional(c.creditUsed),pipeline=optional(c.openPipeline)
  return {currentPurchases:current,previousPurchases:optional(c.purchasePreviousSeason),potentialTotal:potential,openPotential:open,openPipeline:pipeline,weightedPipeline:null,forecast:null,averageTicket:known(c.purchaseTotal)&&known(c.purchaseCount)&&Number(c.purchaseCount)>0?Number(c.purchaseTotal)/Number(c.purchaseCount):null,conversionRate:optional(c.conversion),potentialCoveragePercent:current!==null&&potential>0?current/potential*100:null,pipelineCoveragePercent:pipeline!==null&&open>0?pipeline/open*100:null,creditLimit,creditUsed,creditAvailable:creditLimit!==null&&creditUsed!==null?Math.max(0,creditLimit-creditUsed):null,purchaseGrowthPercent:optional(c.purchaseGrowthPercent),walletShare:optional(c.walletShare),targetShare:optional(c.targetShare),grossMarginPercent:optional(c.grossMarginPercent),estimatedMargin:current!==null&&known(c.grossMarginPercent)?current*Number(c.grossMarginPercent)/100:null,marginTotal:optional(c.marginTotal),wins:optional(c.wins),losses:optional(c.losses),knownOutcomes:optional(c.knownOutcomes),paymentTerms:c.paymentTerms||'',decisionWindow:c.decisionWindow||'',commercialRisk:c.commercialRisk||'',lastPurchaseAt:c.lastPurchaseAt||null}
 },[client])
 ...
 return <section className="producer-business-overview" aria-labelledby="business-overview-title">
  ...
  <div className="business-metric-grid">
   <Metric loading={state.loading} icon={ShoppingCart} label="Compras da safra atual" value={money(business.currentPurchases)} detail={`Safra anterior: ${money(business.previousPurchases)}`}/>
   <Metric loading={state.loading} icon={WalletCards} label="Potencial total" value={money(business.potentialTotal)} detail={`Cobertura: ${percent(business.potentialCoveragePercent)}`}/>
   <Metric loading={state.loading} icon={Target} label="Potencial em aberto" value={money(business.openPotential)} detail="Calculado automaticamente" tone="is-attention"/>
   <Metric loading={state.loading} icon={TrendingUp} label="Previsão com pipeline" value={money(business.forecast)} detail={`Pipeline ponderado: ${money(business.weightedPipeline)}`}/>
   <Metric loading={state.loading} icon={BadgeDollarSign} label="Pipeline aberto" value={money(business.openPipeline)} detail={`Cobertura do gap: ${percent(business.pipelineCoveragePercent)}`}/>
   <Metric loading={state.loading} icon={BarChart3} label="Ticket médio global" value={money(business.averageTicket)} detail={`Conversão: ${percent(business.conversionRate)}`}/>
   <Metric loading={state.loading} icon={WalletCards} label="Crédito disponível" value={money(business.creditAvailable)} detail={`${money(business.creditUsed)} já utilizados do limite`}/>
   <Metric loading={state.loading} icon={CalendarClock} label="Última compra" value={date(business.lastPurchaseAt)} detail={`Variação entre safras: ${percent(business.purchaseGrowthPercent)}`}/>
   <Metric loading={state.loading} icon={BadgeDollarSign} label="Margem estimada da safra" value={business.estimatedMargin===null||business.estimatedMargin===undefined?'Não calculada':money(business.estimatedMargin)} detail={business.grossMarginPercent===null||business.grossMarginPercent===undefined?'Informe a margem bruta no cadastro.':`${percent(business.grossMarginPercent)} sobre a compra atual`}/>
   <Metric loading={state.loading} icon={TrendingUp} label="Margem registrada" value={money(business.marginTotal)} detail="Somada apenas quando o histórico contém margem"/>
   <Metric loading={state.loading} icon={BarChart3} label="Ganhos / perdas" value={`${shown(business.wins)} / ${shown(business.losses)}`} detail={countLabel(business.knownOutcomes,'resultado classificado','resultados classificados')}/>
   <Metric loading={state.loading} icon={CalendarClock} label="Ações vencidas" value={state.data?.pipeline?finite(overdue).toLocaleString('pt-BR'):'—'} detail={overdue?'Requer revisão do próximo compromisso.':'Pipeline sem ação vencida registrada.'} tone={overdue?'is-attention':''}/>
  </div>
  <div className="business-chart-grid">
   <article className="business-chart-card coverage-card"><header><div><small>COBERTURA DO POTENCIAL</small><h4>Realizado × oportunidade em aberto</h4></div><span>{percent(business.potentialCoveragePercent)}</span></header><div className="coverage-visual"><div className="coverage-ring" style={{'--coverage':`${coverage*3.6}deg`}}><span><b>{percent(business.potentialCoveragePercent)}</b><small>realizado</small></span></div><dl><div><dt>Realizado</dt><dd>{money(business.currentPurchases)}</dd></div><div><dt>Em aberto</dt><dd>{money(business.openPotential)}</dd></div><div><dt>Pipeline</dt><dd>{money(business.openPipeline)}</dd></div></dl></div></article>
  ...
  <div className="business-decision-grid">
   ...
   <article><CalendarClock/><span><small>JANELA DE DECISÃO</small><b>{shown(business.decisionWindow)}</b><em>{overdue?`${countLabel(overdue,'próxima ação vencida','próximas ações vencidas')} no funil.`:'Nenhuma próxima ação vencida registrada.'}</em></span></article>
   <article><WalletCards/><span><small>CONDIÇÃO COMERCIAL</small><b>{shown(business.paymentTerms)}</b><em>{!known(business.creditLimit)?'Limite de crédito ainda não informado.':!known(business.creditUsed)?'Crédito utilizado ainda não informado.':`${percent(Number(business.creditUsed)/Number(business.creditLimit)*100)} do limite de crédito utilizado.`}</em></span></article>
  ...
  <article className="manual-business-sync"><header><div><span><DatabaseZap/></span><div><small>MANUAL DO AGRÔNOMO • MESMO LOGIN</small><h4>Contexto técnico sincronizado</h4><p>O preenchimento técnico amplia o dossiê da VAL sem misturar carteiras de outros usuários.</p></div></div><em>{technical.lastSyncAt?`Última sincronização: ${date(technical.lastSyncAt)}`:'Aguardando o primeiro registro vinculado ao produtor'}</em></header>
   <div className="manual-sync-metrics"><div><MapPinned/><span><b>{known(technical.properties)?Number(technical.properties):"—"}</b><small>propriedades</small></span></div><div><Layers3/><span><b>{known(technical.fields)?Number(technical.fields):"—"}</b><small>talhões</small></span></div><div><FileBarChart/><span><b>{known(technical.cropSeasons)?Number(technical.cropSeasons):"—"}</b><small>safras</small></span></div><div><DatabaseZap/><span><b>{known(technical.soilAnalyses)?Number(technical.soilAnalyses):"—"}</b><small>análises de solo</small></span></div><div><FileBarChart/><span><b>{known(technical.fieldReports)?Number(technical.fieldReports):"—"}</b><small>relatórios</small></span></div><div><BarChart3/><span><b>{known(technical.ndvi)?Number(technical.ndvi):"—"}</b><small>leituras NDVI</small></span></div></div>
