...
import {
 ...
 MapPin,Plus,RefreshCw,Route,ShieldCheck,Sparkles,Target,UserRound,Warehouse,X
} from 'lucide-react'
import {requestJsonResource,useAsyncResource} from '../hooks/useAsyncResource'
import SogAnalysisPanel from './SogAnalysisPanel'
...
export default function SogWorkspace({clients=[],onSelect}){
 ...
 const tabs=[['opportunities','Oportunidades',workspace.opportunities?.length||0,Target],['analysis','Análise',null,Sparkles],['intentions','Intenções',summary.activeIntentions||0,Handshake],['market','Mercado',workspace.marketSnapshots?.length||0,LineChart],['producers','Produtores',producers.length,UserRound],['ecosystem','Alimentação',null,Database]]
 ...
 return <section className="sog-workspace" aria-labelledby="sog-title">
  ...
  {tab==='analysis'&&<SogAnalysisPanel producers={producers} catalog={workspace.catalog||{}} profilesByClient={profilesByClient} commodityMap={commodityMap}/>}
