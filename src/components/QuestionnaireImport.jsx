...
export default function QuestionnaireImport({onReview}){
 ...
 const inspect=async file=>{
  ...
  try{const source=await parseImportFile(file);const recognized=recognizeQuestionnaire(source);setReport({...recognized,fileName:file.name})}
 ...
 const inspectGoogle=async()=>{
  ...
  setLoading(true);setError('');setReport(null)
  try{
   const response=await fetch('/api/import/google-sheet',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({url:googleUrl})})
 ...
 return <section className="question-import-studio">
  <div className="import-studio-head"><div><span className="eyebrow">VAL DOCUMENT INTELLIGENCE</span><h2>Importe respostas feitas fora do sistema.</h2><p>A Val reconhece perguntas, alternativas e escalas mesmo quando o arquivo tem outra ordem ou nomenclatura.</p></div><div className="scan-emblem"><ScanSearch/><span>RECONHECIMENTO<br/>MULTIFORMATO</span></div></div>
  ...
  {report&&<div className="recognition-report"><div className="recognition-score"><div style={{'--recognition':`${report.confidence*3.6}deg`}}><span><b>{report.confidence}%</b><small>reconhecido</small></span></div></div><div className="recognition-copy"><span className="eyebrow">LEITURA CONCLUÍDA • {report.format}</span><h3>{report.recognized.length} de 27 respostas identificadas</h3><p>{report.missing.length?`${report.missing.length} campos ainda precisam de revisão antes da compilação final.`:'O diagnóstico está completo e pronto para compilar.'}</p><div className="recognized-chips">{report.recognized.slice(0,5).map(item=><span key={item.id}><CheckCircle2/>{String(item.value).slice(0,32)}</span>)}</div><button className="primary-btn" onClick={()=>onReview(report.answers)}>Revisar, completar e compilar <ArrowRight size={16}/></button></div></div>}
 </section>
}
