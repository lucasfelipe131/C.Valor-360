...
import {cropPotential,emptySeasonCrops,numeric,previousYield,seasonCode,seasonOrder,validSeasonCode} from '../lib/producer-seasons'
...
export default function ProducerSeasons({client,resource}){
 ...
 const codes=[...new Set(['2627V','2727I',...resource.seasons.map(s=>s.season),selected])].sort((a,b)=>seasonOrder(b)-seasonOrder(a))
 const history=resource.seasons.filter(s=>seasonOrder(s.season)<seasonOrder(selected)).sort((a,b)=>seasonOrder(b.season)-seasonOrder(a.season))
 ...
 <div className="season-form-head"><label>Safra<select disabled={busy} value={selected} onChange={e=>switchSeason(e.target.value)}>{codes.map(code=><option key={code}>{code}</option>)}</select></label><label>Outra safra<input disabled={busy} value={newCode} onChange={e=>setNewCode(seasonCode(e.target.value))} placeholder="2526V" maxLength={5}/></label><button disabled={busy||!validSeasonCode(newCode)} onClick={()=>{switchSeason(newCode);setNewCode('')}}>Abrir safra</button><p>V = verão · I = inverno. Ex.: 2627V e 2727I. Use “Outra safra” para registrar o histórico.</p></div>
 <div className="season-form-head"><label className="season-source">Origem dos dados<input disabled={busy} value={draft.sourceNote} onChange={e=>edit({sourceNote:e.target.value})} maxLength={500} placeholder="Conversa com o produtor, levantamento ou documento"/></label><label>Data de referência<input disabled={busy} type="date" value={draft.observedOn} max={new Date().toISOString().slice(0,10)} onChange={e=>edit({observedOn:e.target.value})}/></label></div>
