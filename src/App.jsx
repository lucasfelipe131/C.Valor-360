...
export default function App(){
 ...
 const [routeMapState,setRouteMapState]=useState({scope:'',value:{}})
 ...
 const openClient=(c,{preserveConversation=false,propertyId='',tab}={})=>{
  ...
  const nextTab=tab||(propertyId?'map':'overview')
  if(page==='client360'&&(String(selected?.id)!==String(c.id)||producerPropertyId!==propertyId||producerTab!==nextTab))navigationHistory.current.push(navigationSnapshot())
  setProducerPropertyId(propertyId);setProducerTabRaw(nextTab)
  if(!preserveConversation&&String(c.id)!==copilotClientRef.current)setCopilotSeed({clientId:c.id,nonce:Date.now()})
  setSelected(c);setCopilotRevealKey(value=>value+1);setCopilotLoaded(true)
  setCopilotOpen(current=>current||window.matchMedia('(min-width:1051px)').matches)
 ...
 const openCopilot=(input={})=>{
  setCopilotRevealKey(value=>value+1)
  const request=input&&typeof input==='object'?input:{}
  const requestedClientId=String(request.client?.id||request.clientId||'')
  const hasNewRequest=Object.keys(request).some(key=>!['client','clientId'].includes(key))
  const resume=copilotLoaded&&!hasNewRequest&&(!requestedClientId||(page==='client360'&&requestedClientId===String(selected?.id)))
  if(!resume){
   const launch=resolveCopilotLaunch({input:request,implicitContext:copilotPageContext,page,storageScope:copilotOwnerScope,clients:clientList,selectedClient:selected})
   setCopilotSeed({...launch,nonce:Date.now()})
  }
 ...
 const activeCopilotClient=useMemo(()=>{
  ...
  if(page==='visits'&&prepareVisitClientId)return selected
  return null
 ...
 const cancelVisitResult=visit=>{if(!visit)return;setVisits(current=>[visit,...current.filter(item=>item.id!==visit.id)]);notify('Visita cancelada. Ela saiu da rota e ficou no histórico.')}
 ...
 useEffect(()=>{
  if(authenticated!==true||!portfolioReady||currentUser?.demo||!currentUser?.id||currentUser?.role==='bi_viewer')return
 ...
 },[page,selected?.id,authenticated,portfolioReady,currentUser?.id,currentUser?.demo,currentUser?.role])
 ...
 if(currentUser?.role==='bi_viewer')return <RouteBoundary routeKey={currentUser.storageScope}><Suspense fallback={<RouteFallback/>}><Management currentUser={currentUser} standalone onLogout={logout}/>{toast&&<div className="toast" role="status">{toast}</div>}</Suspense></RouteBoundary>
 ...
 return <div className={`app-shell ${page==='client360'?'p360-shell':''} ${page==='opportunities'?'opp-shell':''}`}>
  ...
  <main className="main" id="main-content" tabIndex="-1">
   ...
   <div className={`content ${page==='copilot'?'content-copilot-fullscreen':''} ${page==='client360'&&copilotOpen?'p360-with-copilot':''} ${page!=='copilot'&&copilotOpen?'val-with-copilot':''}`}>
    ...
    {page==='visits'&&<Visits key={copilotOwnerScope} routeMapState={routeMapState.scope===copilotOwnerScope?routeMapState.value:{}} onRouteMapStateChange={saveRouteMapState} onOpenClient={openClient} clients={clientList} visits={visits} storageScope={currentUser?.storageScope} initialClientId={prepareVisitClientId} initialVisitId={prepareVisitId} onInitialHandled={()=>{setPrepareVisitClientId('');setPrepareVisitId('')}} onSave={saveVisit} onPrepare={openValClient} onAsk={openCopilot} onContextChange={updateCopilotPageContext} onStarted={startVisitResult} onCancelled={cancelVisitResult} onRegistered={registerVisitResult} onRefreshPortfolio={refreshPortfolio}/>}
