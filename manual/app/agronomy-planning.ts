...
const UF_REGION: Record<string, BrazilRegion> = {
 ...
 */
const REGION_CYCLE_DAYS: Record<PlanningCrop, Record<BrazilRegion, number>> = {
...
const REGION_REFERENCE_ABS_LATITUDE: Record<BrazilRegion, number> = {
  Sul: 27,
  Sudeste: 20.5,
  "Centro-Oeste": 16.5,
  Nordeste: 10,
...
const LATITUDE_DAYS_PER_DEGREE: Record<PlanningCrop, number> = {
  Soja: 0.45,
  Milho: 0.25,
  Trigo: 0.2,
  Canola: 0.2,
...
const SEASON_DAYS: Record<PlanningCrop, Record<number, number>> = {
...
const GENERAL_SOWING_MONTHS: Record<PlanningCrop, Record<BrazilRegion, number[]>> = {
  Soja: {
...
const POPULATION_BASE: Record<PlanningCrop, {
  min: number;
  target: number;
  max: number;
...
const POPULATION_REGION_FACTOR: Record<PlanningCrop, Record<BrazilRegion, number>> = {
...
const ENVIRONMENT_FACTOR: Record<PlanningCrop, Record<ProductionEnvironment, number>> = {
...
function validBrazilLatitude(value: number | null | undefined) {
  const latitude = Number(value);
  return Number.isFinite(latitude) && latitude >= -34 && latitude <= 6 ? latitude : null;
}

export function regionForUf(uf: string): BrazilRegion {
  return UF_REGION[String(uf || "").toUpperCase()] ?? "Sul";
}

export function locationLabel(municipality: string, uf: string) {
...
export function estimateRegionalHarvest(input: {
  ...
  const municipalityAdjustmentDays = municipalityLatitude === null
    ? 0
    : Math.round(clamp(
        (Math.abs(municipalityLatitude) - REGION_REFERENCE_ABS_LATITUDE[region])
          * LATITUDE_DAYS_PER_DEGREE[input.crop],
        -3,
        3,
      ));
  const harvestConditionDays = Math.max(0, Math.round(input.harvestConditionDays ?? 0));
  const physiologicalCycleDays = Math.max(
    45,
    baseCycleDays + regionalAdjustmentDays + municipalityAdjustmentDays + seasonAdjustmentDays,
  );
  ...
  const startCycleDays = Math.max(
    40,
    catalogMin + regionalAdjustmentDays + municipalityAdjustmentDays + seasonAdjustmentDays + harvestConditionDays - regionalUncertainty,
  );
  const endCycleDays = Math.max(
    startCycleDays + 7,
    catalogMax + regionalAdjustmentDays + municipalityAdjustmentDays + seasonAdjustmentDays + harvestConditionDays + regionalUncertainty,
  );
...
function materialFactor(crop: PlanningCrop, cultivar: PlanningCultivar) {
  const cycle = (cultivar.cycleClass ?? "").toLocaleLowerCase("pt-BR");
  if (crop === "Soja") {
    if (cycle.includes("superprecoce")) return 1.05;
    if (cycle.includes("precoce")) return 1.025;
    if (cycle.includes("tard")) return 0.97;
  }
  return 1;
}
...
export function recommendPlantPopulation(input: {
  ...
  const combinedFactor =
    POPULATION_REGION_FACTOR[input.crop][region]
  ...
  if (outsideGeneralWindow) warnings.unshift("Plantio fora do calendário geral da cultura/macrorregião: confirme ZARC e reduza a confiança da população calculada.");
  if (gap > 25) warnings.push("Yield gap acima de 25%: diagnostique primeiro os fatores limitantes e valide a densidade em faixa lado a lado.");
  if (input.germinationPercent < 80 || input.emergencePercent < 80) warnings.push("Germinação ou emergência abaixo de 80% eleva muito a compensação; confira teste do lote e condição do sulco.");
  if (spacingCm < base.spacing[0] || spacingCm > base.spacing[1]) warnings.push(`Espaçamento fora da faixa geral de ${base.spacing[0]}–${base.spacing[1]} cm usada para ${input.crop.toLocaleLowerCase("pt-BR")}.`);
  if ((input.crop === "Trigo" || input.crop === "Canola") && (region === "Norte" || region === "Nordeste")) warnings.push("A adaptação regional desta cultura precisa ser confirmada em pesquisa/assistência local antes de definir população.");
