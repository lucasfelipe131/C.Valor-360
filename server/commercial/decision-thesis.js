...
export function buildDecisionThesis(input={}){
 ...
 const highestUnknown=text(input?.decisionIntelligence?.highest_value_unknown?.question||input?.decisionIntelligence?.highestValueUnknown?.question||advice?.next_question?.question||missing[0])
 const recommendedAction=mustDiscover
 ...
 return assertContract({
  ...
  objective:text(input.objective||snapshot.objective||advice.objective||'Melhorar a próxima decisão comercial.'),
  ...
  next_action:mustDiscover?(highestUnknown||'Coletar e confirmar o dado crítico antes de propor.'):(legacyAction||'Registrar o próximo passo com responsável e data.')
