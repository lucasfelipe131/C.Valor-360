...
function validateAudit(audit,violations){
  ...
  exactKeys(audit,new Set(['contract_version','request_id','organization_id','actor_ref','route_id','objective','planned_modules','module_runs','commercial_modules','execution_modules','behavioral_profile_version','decision_thesis_version','value_plan_version','action_plan_id','action_plan_version','context_snapshot_id','policy_decision','started_at','completed_at']),violations,'audit')
  ...
  if(audit.execution_modules!==undefined)push(violations,Array.isArray(audit.execution_modules)&&audit.execution_modules.every(item=>['MEX','VIS'].includes(item)),'audit.execution_modules')
  for(const key of ['behavioral_profile_version','decision_thesis_version','value_plan_version','action_plan_id','action_plan_version','context_snapshot_id'])if(audit[key]!==undefined&&audit[key]!==null)push(violations,text(audit[key],180),`audit.${key}`)
