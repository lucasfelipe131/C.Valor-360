 ...
 return Object.freeze({
  ...
  async confirm({tenantId,ownerId,actorId=ownerId,id,input={},requestId,now}={}){
   ...
   if(interaction.interaction_type==='POST_VISIT'){
    ...
    if(!current){const transcript=interaction.transcript;if(!transcript?.transcript_text)throw voiceError('A transcrição confirmada não está disponível para criar o Visit Report.','voice_transcript_unavailable',422);const created=await visitLoop.createReportFromTranscript({tenantId,ownerId,actorId,visitId:interaction.visit_id,transcriptText:transcript.transcript_text,transcriptRef:interaction.transcript_ref,voiceInteractionId:id,requestId,now});current=created.visit_report;reportId=current.visit_report_id}
