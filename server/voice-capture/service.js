 ...
 return Object.freeze({
  ...
  async confirm({tenantId,ownerId,actorId=ownerId,id,input={},requestId,now}={}){
   ...
   if(interaction.interaction_type==='POST_VISIT'){
    ...
    const confirmedForReport=latest=>latest.state==='CONFIRMED'&&String(latest.related_artifacts?.visit_report_id||'')===String(reportId)&&String(latest.visit_id)===String(interaction.visit_id)
    const confirmedResponse=async latest=>({contract_version:'val.voice_confirmation.response.v1',voice_interaction:latest,result:await repository.getVisitLearningContext({tenantId,ownerId,visitId:latest.visit_id}),idempotent:true,message:'Esta interação já foi confirmada.'})
    if(current.confirmation_status==='CONFIRMED'){
     // A concurrent request may have committed the report and this exact voice
     // interaction after our first read. Recheck the authorized interaction;
     // a report confirmed by another flow must still leave candidates pending.
     const latest=await get({tenantId,ownerId,actorId,id})
     if(confirmedForReport(latest))return confirmedResponse(latest)
     throw voiceError('O Visit Report vinculado já foi confirmado por outro fluxo. Esta interação permaneceu pendente para evitar aplicar candidatos sem atomicidade.','voice_visit_report_already_confirmed',409)
    }
    else{const mappedCandidateIds=new Set([fields.objections,fields.producer_signals,fields.expectations_created,fields.opportunities_detected,fields.commitments_confirmed,fields.next_steps,fields.technical_observations,fields.behavioral_signals,fields.missing_information].flat().map(item=>String(item.item_id)));const voiceMemories=memoryWrites(interaction,confirmed.filter(item=>['FACT_CANDIDATE','HYPOTHESIS'].includes(item.category)&&!mappedCandidateIds.has(String(item.candidate_id))),actorId,now);try{result=await visitLoop.confirmReport({tenantId,ownerId,actorId,visitId:interaction.visit_id,input:{visit_report_id:reportId,fields,commitments_confirmed:fields.commitments_confirmed,next_steps:fields.next_steps,outcome_type:String(input.outcome_type||'NO_DECISION').toUpperCase(),result:{summary:fields.summary},evidence_refs:[{id:`voice-interaction:${id}`,type:'confirmed_voice_interaction'}]},voiceConfirmation:{interaction_id:id,reviewed_candidates:reviewed,memory_writes:voiceMemories,related_artifacts:interaction.related_artifacts},requestId,now})}catch(error){const latest=await get({tenantId,ownerId,actorId,id});if(error?.statusCode===409&&confirmedForReport(latest))return confirmedResponse(latest);throw error}}
