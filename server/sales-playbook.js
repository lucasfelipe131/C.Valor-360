...
export function buildFallbackAdvice({client={},message='',signals=[],learning={}}){
  ...
  const opportunity=client.commercial?.opportunity||signals[0]?.title||'definir a prioridade técnica e econômica'
  const evidenceUsed=[]
  ...
  const nextQuestion={stage:'problema',question:`Onde “${String(opportunity).toLowerCase()}” afeta uma decisão concreta hoje?`,ask_when:'Depois de confirmar objetivo, tempo e permissão para usar os dados.',purpose:'Validar se a hipótese cadastrada é uma prioridade real.',evidence_needed:'Exemplo recente, área afetada, frequência e decisão alterada.'}
  return {
    audience:'internal',safe_to_show_customer:false,
    answer:`Para a conversa com ${firstName(client.name)}, valide primeiro se “${String(opportunity).toLowerCase()}” é uma prioridade real. O modo demonstrativo organiza o processo, mas não produz análise generativa nem diagnóstico.`,
    objective:`Confirmar o problema, a linha de base e qual evidência tornaria uma mudança justificável.`,
    ...
    questions:[nextQuestion,{stage:'implicação',question:'Como você compara os riscos de agir agora, esperar e manter a prática?',ask_when:'Somente depois de o problema ser reconhecido.',purpose:'Comparar alternativas sem pressupor que mudar é melhor.',evidence_needed:'Custos, janela, probabilidade, reversibilidade e impacto de cada alternativa.'},{stage:'necessidade',question:'Que resultado e método de medição fariam um teste valer a pena?',ask_when:'Depois da comparação simétrica.',purpose:'Definir valor e prova na linguagem do produtor.',evidence_needed:'Métrica, linha de base, horizonte e critério de interrupção.'}],
    ...
    value_hypothesis:{problem:opportunity,baseline:'Não confirmada.',act_now:'Medir custo total, benefício possível, risco e reversibilidade.',wait:'Medir o valor da informação adicional e o risco de perder a janela.',maintain:'Medir desempenho, custo e risco da prática atual sem assumir perda.',impact_to_quantify:'R$/ha, sc/ha, horas, janela, probabilidade e risco operacional.',value_metric:'Valor realizado contra a mesma linha de base e alternativa.',time_horizon:'Definir com o produtor.',proof_plan:'Comparação controlada, premissas registradas e revisão técnica quando aplicável.',double_counting_guard:'Não somar o mesmo efeito como receita incremental e perda evitada.',uncertainty:'Sem linha de base, contrafactual e horizonte não existe estimativa defensável.'},
    next_best_action:'Abrir a conversa com OPC e fazer somente a próxima pergunta necessária.',
    commitment:null,
