...
const clientNameOf=item=>text(item?.client_name||item?.clientName,180)
...
export function buildObjectionLibrary(context={},options={}){
 ...
 const hasFocus=Boolean(focus.title||focus.category||focus.product)
 const history=array(options.portfolioHistory||context.portfolioBusinessHistory||context.businessHistory)
 const losses=history.filter(item=>isLoss(item)&&timestamp(eventDate(item))!==null&&timestamp(eventDate(item))>=horizonStart&&timestamp(eventDate(item))<=now).map(item=>({item,similarity:similarityToFocus(item,focus)})).filter(entry=>hasFocus?entry.similarity.score>=3:true)
 ...
 const objections=[...groups.values()].map(group=>{
  ...
  const move=sorted.map(loss=>observedMoveFor(loss,history,context.priorRecommendations,now)).find(Boolean)||null
  return {
...
export async function loadPortfolioBusinessHistory(repository,ownerId,options={}){
 ...
 if(repository?.db?.configured){
  ...
  events=result.rows.map(row=>({id:row.id,source:row.source,externalId:row.external_id,occurredAt:iso(row.occurred_at),outcome:row.outcome,category:row.category,product:row.product,value:row.value==null?null:Number(row.value),lossReason:row.loss_reason,payload:object(row.payload),clientExternalKey:row.client_external_key,clientName:row.client_name}))
 }else{
