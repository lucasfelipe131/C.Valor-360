...
if(!globalThis[PATCHED]){
 ...
 ValRepository.prototype.getClientContext=async function contextWithConversionInnovations(input){
  ...
  try{
   portfolioHistory=await loadPortfolioBusinessHistory(this,input?.ownerId)
  }catch(error){
   objectionLibraryLoadError=String(error?.message||'A biblioteca da carteira não pôde ser consultada.').slice(0,300)
  }
  return {
   ...context,
   conversionInnovations:{
    ...
    objectionLibrary:{
     ...buildObjectionLibrary(context,{portfolioHistory}),
     loadError:objectionLibraryLoadError
    }
   }
  }
 }
