...
export function buildMessageCalibration(context={},options={}){
 ...
 return {version:'val-message-calibration-v2',generatedAt:new Date(now).toISOString(),mode:'shadow',lookbackDays,minSample,sampleStatus:readySegments?'partially_ready':'building',readySegments,summary,segments,messages:messages.slice(0,16),messagesTotal:messages.length,messagesHidden:Math.max(0,messages.length-16),recentObservations:observations.slice(-8).reverse(),policy:{freeNotesExcluded:true,automaticPromptChange:false,productionRanking:false,causalClaims:false,personalDataFeatures:false,minimumSampleForComparison:minSample},interpretation:summary.nextInteractionsObserved?'Avanço significa que a etapa metodológica registrada na conversa seguinte ficou à frente da etapa anterior. A sequência temporal não prova que a frase causou o avanço.':'Ainda não existem interações seguintes suficientes para medir coincidência com avanço metodológico.',guardrail:'O placar é descritivo e permanece em shadow mode. Ele não altera prompts, score, rota, próxima ação ou linguagem da VAL sem avaliação offline, amostra mínima e aprovação humana.',emptyReason:messages.length?'':'Ainda não há mensagens estruturadas suficientes para formar o placar.'}
}
