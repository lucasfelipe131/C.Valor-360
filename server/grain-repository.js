...
const ownerMatches=(item,ownerId)=>String(item.ownerId??'')===String(ownerId??'')
...
export class GrainRepository{
 ...
 fallbackProducers(store,ownerId){
  ...
  for(const record of store.imports||[]){if(record.ownerId!=null&&!ownerMatches(record,ownerId))continue;for(const client of record.clients||[])if(client?.id||client?.name)producers.set(String(client.id||client.name),{id:String(client.id||client.name),name:client.name||'Produtor',municipality:client.municipality||'',cultures:client.cultures||'',area:client.area??null,profileId:null})}
  for(const profile of store.grains.profiles||[])if(ownerMatches(profile,ownerId)&&profile.clientId&&!producers.has(String(profile.clientId)))producers.set(String(profile.clientId),{id:String(profile.clientId),name:profile.clientName||'Produtor',municipality:profile.municipality||'',cultures:'',area:null,profileId:profile.id})
 ...
 async getWorkspace(ownerId){
  if(!this.db.configured){
   const store=this.fallback();const profiles=store.grains.profiles.filter(item=>ownerMatches(item,ownerId)).map(profileRecord);const intentions=store.grains.intentions.filter(item=>ownerMatches(item,ownerId)).map(intentRecord);const marketSnapshots=store.grains.marketSnapshots.filter(item=>ownerMatches(item,ownerId)).map(marketRecord).sort((left,right)=>String(right.observedAt).localeCompare(String(left.observedAt)));return this.assemble({producers:this.fallbackProducers(store,ownerId),profiles,intentions,marketSnapshots})
  }
  try{
   const [producerResult,intentResult,marketResult]=await Promise.all([
    this.db.query(`SELECT c.id,c.external_key,c.name,c.municipality,c.cultures,c.total_area_ha,c.area_band,p.id profile_id,p.commodities,p.storage_capacity_t,p.storage_structure,p.logistics_mode,p.usual_delivery_locations,p.marketing_notes,p.source,p.source_details,p.observed_at,p.confirmed_at,p.created_at,p.updated_at FROM clients c LEFT JOIN sog_producer_profiles p ON p.tenant_id=c.tenant_id AND p.owner_user_id=$2 AND p.client_id=c.id WHERE c.tenant_id=$1 AND c.consultant_id=$2 AND c.status='active' ORDER BY c.name LIMIT 5000`,[this.tenantId,ownerId]),
    ...
    this.db.query(`SELECT * FROM sog_market_snapshots WHERE tenant_id=$1 AND owner_user_id=$2 AND status='active' ORDER BY observed_at DESC LIMIT 1000`,[this.tenantId,ownerId])
   ])
   const producers=producerResult.rows.map(producerRecord)
   const profiles=producerResult.rows.filter(row=>row.profile_id).map(row=>profileRecord({...row,id:row.profile_id,client_external_key:row.external_key,client_name:row.name}))
 ...
 async saveProfile(input,ownerId){
  ...
  if(!this.db.configured){
   const store=this.fallback();const current=store.grains.profiles.find(item=>ownerMatches(item,ownerId)&&item.clientId===input.clientId);const producer=this.fallbackProducers(store,ownerId).find(item=>item.id===input.clientId);if(!producer&&!current)throw domainError('Produtor não encontrado na sua carteira.',404);const record={...current,...input,id:current?.id||randomUUID(),ownerId,clientName:producer?.name||current?.clientName||'Produtor',municipality:producer?.municipality||current?.municipality||'',confirmedAt:input.confirmed?now:null,createdAt:current?.createdAt||now,updatedAt:now};store.grains.profiles=store.grains.profiles.filter(item=>!(ownerMatches(item,ownerId)&&item.clientId===input.clientId)).concat(record).slice(-2000);this.saveStore(store);return profileRecord(record)
  }
 ...
 async saveIntent(input,ownerId){
  ...
  if(!this.db.configured){
   const store=this.fallback();const producer=this.fallbackProducers(store,ownerId).find(item=>item.id===input.clientId);if(!producer)throw domainError('Produtor não encontrado na sua carteira.',404);const record={...input,id,ownerId,clientName:producer.name,municipality:producer.municipality||'',createdAt:now,updatedAt:now};store.grains.intentions.push(record);store.grains.intentions=store.grains.intentions.slice(-5000);this.saveStore(store);return intentRecord(record)
  }
  try{return await this.db.transaction(async connection=>{
 ...
 async updateIntentStatus(id,status,ownerId){
  if(!this.db.configured){const store=this.fallback();const record=store.grains.intentions.find(item=>ownerMatches(item,ownerId)&&item.id===id);if(!record)throw domainError('Intenção não encontrada.',404);if(record.status!==status&&!allowedIntentTransitions[record.status]?.has(status))throw domainError('Esta mudança de estado exige uma nova validação da intenção.',409);record.status=status;record.updatedAt=new Date().toISOString();this.saveStore(store);return intentRecord(record)}
