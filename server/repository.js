...
export class ValRepository{
  ...
  async confirmVoiceInteraction({tenantId=this.tenantId,ownerId,actorId=ownerId,interactionId,reviewedCandidates=[],summary='',memories=[],commitments=[],opportunities=[],relatedArtifacts={},requestId,now=new Date().toISOString()}={}){
    ...
    if(!this.db.configured){
      ...
      const storedOpportunities=[];for(const item of opportunities){let existing=store.opportunities.find(entry=>String(entry.clientId)===String(voice.client_external_key||voice.client_id)&&String(entry.candidateKey)===String(item.candidate_key));if(!existing){existing={id:`o-${randomUUID()}`,clientId:voice.client_external_key||voice.client_id,title:item.title,category:item.category,hypothesis:item.hypothesis,value:Number(item.estimated_value||0),stage:item.stage||'Diagnóstico',nextAction:item.next_action||'',nextActionAt:item.next_action_at||null,candidateKey:item.candidate_key,evidence:structuredClone(item.evidence||[]),updatedAt:now};store.opportunities.push(existing)}storedOpportunities.push(existing)}
      const artifacts={...structuredClone(relatedArtifacts),interaction_id:interaction.id,memory_ids:memoryIds,commitment_ids:storedCommitments.map(item=>item.commitment_id),opportunity_ids:storedOpportunities.map(item=>item.id)};Object.assign(voice,{status:'CONFIRMED',confirmation_status:'CONFIRMED',reviewed_candidates:structuredClone(reviewedCandidates),related_artifacts:artifacts,confirmed_at:now,processed_at:voice.processed_at||now,revision_no:Number(voice.revision_no||1)+1,error_code:null,error_message:null,updated_at:now});const attachment=store.val.attachments.find(item=>String(item.id)===String(voice.audio_attachment_id));if(attachment){attachment.status='confirmed';attachment.confirmed_at=now;attachment.updated_at=now}this.saveStore(store);return {voice_interaction:voiceInteractionRecord(voice),interaction:structuredClone(interaction),commitments:structuredClone(storedCommitments),opportunities:structuredClone(storedOpportunities),memories_written:memoryIds,idempotent:false}
    }
  ...
  async confirmVisitReport({tenantId=this.tenantId,ownerId,actorId=ownerId,report,commitments=[],memories=[],opportunities=[],outcome,learningCandidate,reflection={},voiceConfirmation=null,requestId,now}={}){
    ...
    if(!this.db.configured){
      ...
      for(const item of opportunities){const existing=store.opportunities.find(entry=>entry.clientId===report.client_id&&entry.candidateKey===item.candidate_key);if(!existing)store.opportunities.push({id:`o-${randomUUID()}`,clientId:report.client_id,title:item.title,category:item.category,hypothesis:item.hypothesis,value:0,stage:item.stage,nextAction:item.next_action,nextActionAt:item.next_action_at,candidateKey:item.candidate_key,evidence:item.evidence,updatedAt:report.confirmed_at})}
  ...
  async getTechnicalContext(clientId,ownerId){
    ...
    try{const result=await this.db.query(`SELECT m.value,m.status,m.updated_at FROM clients c LEFT JOIN LATERAL (SELECT value,status,updated_at FROM val_memories WHERE tenant_id=c.tenant_id AND client_id=c.id AND key='consultant_technical_context' AND status IN ('proposed','verified') AND (valid_until IS NULL OR valid_until>NOW()) ORDER BY valid_from DESC,updated_at DESC LIMIT 1) m ON true WHERE c.tenant_id=$1 AND c.consultant_id=$3 AND (c.id::text=$2 OR c.external_key=$2) LIMIT 1`,[this.tenantId,clientId,ownerId]);if(!result.rowCount)throw domainError('Cliente não encontrado na base autorizada.',404);return result.rows[0].value?{...result.rows[0].value,status:result.rows[0].status,updatedAt:iso(result.rows[0].updated_at)}:null}catch(error){if(error.statusCode)throw error;throw serviceError('O complemento técnico não pôde ser lido no PostgreSQL configurado.')}
  }
      ...
      try{
        return await this.db.transaction(async client=>{
          ...
          if(event.type==='manual.producer.updated'&&event.clientExternalKey){
            ...
            await client.query(`UPDATE clients SET name=COALESCE(NULLIF($4,''),name),municipality=COALESCE($5,municipality),total_area_ha=COALESCE($6,total_area_ha),area_band=COALESCE($7,area_band),cultures=COALESCE($8,cultures),preferred_channel=COALESCE($9,preferred_channel),updated_at=NOW() WHERE tenant_id=$1 AND consultant_id=$2 AND external_key=$3 AND status='active'`,[tenantId,ownerId,event.clientExternalKey,String(producer.name||producer.producerName||'Produtor').slice(0,180),String(producer.city||producer.municipality||'').slice(0,140)||null,area.totalAreaHa,area.areaBand,String(cultures||'').slice(0,1000)||null,String(producer.preferredChannel||producer.servicePreference||'').slice(0,60)||null])
          }
          const resolvedClient=event.clientExternalKey?await client.query(`SELECT id FROM clients WHERE tenant_id=$1 AND consultant_id=$2 AND (id::text=$3 OR external_key=$3) LIMIT 1`,[tenantId,ownerId,event.clientExternalKey]):{rows:[]}
          const resolvedClientId=resolvedClient.rows[0]?.id||null
          const resolvedProperty=event.propertyExternalKey?await client.query(`SELECT property.id FROM properties property JOIN clients account ON account.id=property.client_id AND account.tenant_id=property.tenant_id WHERE property.tenant_id=$1 AND account.consultant_id=$2 AND (property.id::text=$3 OR property.external_key=$3) LIMIT 1`,[tenantId,ownerId,event.propertyExternalKey]):{rows:[]}
          const resolvedPropertyId=resolvedProperty.rows[0]?.id||null
          const resolvedField=event.fieldExternalKey?await client.query(`SELECT field.id FROM fields field JOIN properties property ON property.id=field.property_id AND property.tenant_id=field.tenant_id JOIN clients account ON account.id=property.client_id AND account.tenant_id=property.tenant_id WHERE field.tenant_id=$1 AND account.consultant_id=$2 AND (field.id::text=$3 OR field.external_key=$3) LIMIT 1`,[tenantId,ownerId,event.fieldExternalKey]):{rows:[]}
