...
import {transitionVisitLifecycle} from './visit-loop/lifecycle.js'
...
export class ValRepository{
  ...
  async getVisitPreparation({tenantId=this.tenantId,ownerId,visitId}){
    tenantId=assertTenantScope(this.tenantId,tenantId)
    if(!this.db.configured){const plan=this.fallback().val.actionPlans.filter(item=>String(item.tenantId)===String(tenantId)&&String(item.ownerId)===String(ownerId)&&String(item.visit_id)===String(visitId)).at(-1);return plan?.preparation?{action_plan:structuredClone(plan),preparation:structuredClone(plan.preparation)}:null}
    try{
      const result=await this.db.query(`SELECT plan.*,client.external_key client_external_key FROM val_action_plans plan JOIN clients client ON client.tenant_id=plan.tenant_id AND client.id=plan.client_id JOIN visits visit ON visit.tenant_id=plan.tenant_id AND visit.id=plan.visit_id AND visit.client_id=plan.client_id WHERE plan.tenant_id=$1 AND plan.visit_id=$2 AND plan.owner_user_id=$3 AND visit.consultant_id=$3 AND client.consultant_id=$3 ORDER BY plan.created_at DESC LIMIT 1`,[tenantId,visitId,ownerId])
  ...
  async confirmVisitReport({tenantId=this.tenantId,ownerId,actorId=ownerId,report,commitments=[],memories=[],opportunities=[],outcome,learningCandidate,reflection={},requestId,now}={}){
    tenantId=assertTenantScope(this.tenantId,tenantId)
    ...
    if(!this.db.configured){
      ...
      if(store.val.visitReports[index].confirmation_status==='CONFIRMED')return this.getVisitLearningContext({tenantId,ownerId,visitId:report.visit_id})
    ...
    try{return await this.db.transaction(async connection=>{
      ...
      if(row.confirmation_status==='CONFIRMED')throw domainError('Este report já foi confirmado.',409)
      ...
      return {visit:visitRecord({...updatedVisit.rows[0],client_external_key:row.client_external_key}),visit_report:visitReportRecord({...updated.rows[0],client_external_key:row.client_external_key}),interaction:{...interaction.rows[0],clientId:row.client_external_key},commitments:storedCommitments,outcome:outcomeRecord({...storedOutcome.rows[0],client_external_key:row.client_external_key}),learning_candidate:learningCandidateRecord(storedLearning.rows[0]),reflection,memories_written:memories.map(item=>item.id),opportunities_written:storedOpportunities.map(item=>item.databaseId)}
