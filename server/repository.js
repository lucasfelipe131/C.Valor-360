...
export class ValRepository{
  ...
  async getFastClientFacts({tenantId=this.tenantId,clientId,ownerId,now=new Date(),dataPath=null,timeoutMs}={}){
    ...
    if(!this.db.configured){
      ...
      // "Qual o compromisso pendente?" devolvia o mais recente por data, mesmo concluído, e
      // escondia o que estava aberto. O caminho DEEP já só monta evidência de compromisso aberto;
      // o FAST passa a ordenar os abertos na frente, mantendo a data como desempate.
      const latestCommitment=needs('LATEST_COMMITMENT')?(store.val.commitments||[]).filter(item=>exactScope(item,tenantId,ownerId)&&String(item.client_id??item.clientId)===String(clientId)).sort((left,right)=>(commitmentClosed(left)-commitmentClosed(right))||((timestamp(right.updated_at??right.updatedAt??right.created_at??right.createdAt)||0)-(timestamp(left.updated_at??left.updatedAt??left.created_at??left.createdAt)||0)))[0]||null:null
