...
// A identidade do produtor vinda do questionario era so o nome. Dois xaras - comum no campo, e
// frequente em carteira herdada - colapsavam num cadastro so: o segundo sobrescrevia o primeiro,
// sem aviso e com 200 {saved:true} como se fosse cadastro novo. Medido em Postgres real: o Jose da
// Silva de 1.800 ha em Sorriso vira Lucas do Rio Verde, 120 ha, e as 45 respostas dele ficam orfas
// em client_profiles. A consultora perde um produtor inteiro e a VAL passa a preparar visita e
// recomendar produto com os dados da pessoa errada.
// A regra e a MESMA que server/survey-import.js ja aplica na planilha, e agora vale para os tres
// caminhos: mesmo nome e mesmo municipio (ou municipio ausente de um dos lados) e a mesma pessoa;
...
const resolveSurveyExternalKey=async(connection,tenantId,ownerId,candidateKey,name,municipality='')=>{
  ...
  // do NOME, entao "external_key igual" nao distingue o mesmo produtor reenviando de um xara: era
  // exatamente por ali que o segundo Jose da Silva sobrescrevia o primeiro.
  for(const row of existing.rows){
    const rowPlace=surveyPlaceSlug(row.municipality)
    if(!place||!rowPlace||rowPlace===place)return String(row.external_key).slice(0,180)
  }
...
// "A definir" e o preenchimento automatico que o proprio produto grava quando o municipio fica em
// branco. Gravado na coluna real clients.municipality, ele saia na visao gerencial e no CSV do
...
// Compromisso encerrado: o FAST ordena os abertos na frente para não apresentar um concluído
// como se fosse a pendência do produtor.
const CLOSED_COMMITMENT_STATUS=new Set(['COMPLETED','CANCELLED','CANCELED','REJECTED','DONE','CONCLUIDO','CANCELADO'])
const commitmentClosed=item=>CLOSED_COMMITMENT_STATUS.has(String(item?.status??'').trim().toUpperCase())?1:0
...
// A oportunidade principal do produtor mantém o id `o-<produtor>` que o pipeline do browser usa como
// identidade; oportunidades derivadas de relato de visita ou captura de voz (external_key
// `visit-report:`/`voice:`) recebem id próprio, senão duas linhas do mesmo produtor colidem e
...
export class ValRepository{
  ...
  async integrateSurvey(token,ownerId){
    ...
    try{return await this.db.transaction(async connection=>{
      ...
      if(survey.status!=='integrado'){
        const result=sanitizeProfileResult(survey.result);let externalKey=String(result.id||normalize(result.name).replace(/\s+/g,'-')||randomUUID()).slice(0,180);externalKey=await resolveSurveyExternalKey(connection,this.tenantId,ownerId,externalKey,result.name,result.municipality);const area=parseCultivatedArea(result.area)
  ...
  async saveSurveyProfile({answers,result,source='assisted_survey'},ownerId){
    ...
    try{return await this.db.transaction(async connection=>{
      ...
      externalKey=await resolveSurveyExternalKey(connection,this.tenantId,ownerId,externalKey,result.name,result.municipality)
  ...
  async getVisit({tenantId=this.tenantId,ownerId,id}){
    ...
    // id fora do formato UUID nunca chega à coluna uuid (22P02 virava 503); responde como não encontrada.
  ...
  async confirmVisitReport({tenantId=this.tenantId,ownerId,actorId=ownerId,report,commitments=[],memories=[],opportunities=[],outcome,learningCandidate,reflection={},voiceConfirmation=null,requestId,now}={}){
    ...
    if(!this.db.configured){
      ...
      for(const item of opportunities){const existing=store.opportunities.find(entry=>exactScope(entry,tenantId,ownerId)&&entry.clientId===report.client_id&&entry.candidateKey===item.candidate_key);if(!existing)store.opportunities.push({id:`o-${randomUUID()}`,tenantId,ownerId,clientId:report.client_id,title:item.title,category:item.category,hypothesis:item.hypothesis,value:item.estimated_value??null,valueKnown:item.estimated_value!=null,stage:item.stage,nextAction:item.next_action,nextActionAt:item.next_action_at,candidateKey:item.candidate_key,evidence:item.evidence,updatedAt:report.confirmed_at})}
  ...
  async saveOpportunity(input,ownerId){
    ...
    const externalKey=derivedOpportunityKey.test(candidateKey)?candidateKey.slice(0,180):`pipeline:${createHash('sha256').update(`${input.clientId}:${candidateKey}`).digest('hex').slice(0,64)}`
  ...
  async getFastClientFacts({tenantId=this.tenantId,clientId,ownerId,now=new Date(),dataPath=null,timeoutMs}={}){
    ...
    if(!this.db.configured){
      ...
      const scopedVisits=needs('LATEST_VISIT')||needs('NEXT_SCHEDULED_VISIT')||needs('LATEST_VISIT_CONFIRMED_OBJECTION')?(store.visits||[]).filter(item=>exactScope(item,tenantId,ownerId)&&String(item.clientId??item.client_id)===String(clientId)):[]
      ...
      // Duas perguntas diferentes, dois candidatos: "qual foi o último compromisso?" quer o mais
      // recente por data, e "qual o compromisso pendente?" quer o mais recente ENTRE OS ABERTOS.
      // Um único registro ordenado por status fazia a pergunta neutra mentir (chamava de "último"
      // um registro antigo, só por estar aberto) ou a pergunta pendente esconder o que estava aberto.
      const scopedCommitments=needs('LATEST_COMMITMENT')?(store.val.commitments||[]).filter(item=>exactScope(item,tenantId,ownerId)&&String(item.client_id??item.clientId)===String(clientId)).sort((left,right)=>(timestamp(right.updated_at??right.updatedAt??right.created_at??right.createdAt)||0)-(timestamp(left.updated_at??left.updatedAt??left.created_at??left.createdAt)||0)):[]
      const latestCommitment=scopedCommitments[0]||null
      const latestOpenCommitment=scopedCommitments.find(item=>!commitmentClosed(item))||null
    ...
    try{
      ...
      if(needs('LATEST_COMMITMENT'))select.push(
        `(SELECT row_to_json(k) FROM (SELECT commitment.id commitment_id,commitment.description,commitment.due_at,commitment.status,commitment.success_criteria,commitment.created_at,commitment.updated_at FROM val_commitments commitment WHERE commitment.tenant_id=c.tenant_id AND commitment.client_id=c.id ORDER BY COALESCE(commitment.updated_at,commitment.created_at) DESC,commitment.id DESC LIMIT 1) k) latest_commitment`,
        `(SELECT row_to_json(k) FROM (SELECT commitment.id commitment_id,commitment.description,commitment.due_at,commitment.status,commitment.success_criteria,commitment.created_at,commitment.updated_at FROM val_commitments commitment WHERE commitment.tenant_id=c.tenant_id AND commitment.client_id=c.id AND UPPER(COALESCE(commitment.status,'')) NOT IN ('COMPLETED','CANCELLED','CANCELED','REJECTED','DONE','CONCLUIDO','CANCELADO') ORDER BY COALESCE(commitment.updated_at,commitment.created_at) DESC,commitment.id DESC LIMIT 1) k) latest_open_commitment`)
  ...
  async getClientContext({tenantId=this.tenantId,clientId,client={},ownerId,contextRequest={}}){
    ...
    if(!this.db.configured){
      ...
      // Paridade com o PostgreSQL: a evidência de perfil chega com escopo (tenant/produtor/owner) e o
      // histórico comercial do produtor entra no contexto; antes o demo nunca tinha compras nem
      // sinais comportamentais no caminho DEEP/CONTEXT.
      // O chamador passa só a referência autorizada ({id,name}); o registro completo do produtor
      // (perfil, datas, evidência) vive na importação do store, como o PostgreSQL carrega da tabela.
      const storedClients=new Map();store.imports?.forEach(record=>{if(!exactScope(record,tenantId,ownerId))return;record.clients?.forEach(item=>{if(exactScope(item,tenantId,ownerId,record))storedClients.set(String(item.id),item)})})
      const storedClient=storedClients.get(String(clientId))||null
      const fullClient=storedClient?{...client,...structuredClone(storedClient)}:client
      const scopedProfileEvidence=(fullClient.profileEvidence||[]).map(item=>({tenant_id:tenantId,producer_id:clientId,context_owner_id:ownerId,...item}))
      const scopedContextClient={...fullClient,profileEvidence:scopedProfileEvidence}
      const businessHistory=(store.businessEvents||store.business_events||[]).filter(item=>exactScope(item,tenantId,ownerId)&&String(item.clientId??item.client_id)===String(clientId)).slice(-50)
      const context={client:scopedContextClient,profile:{answers:fullClient.profileAnswers||{},evidence:scopedProfileEvidence,assessedAt:fullClient.profileUpdatedAt||null,validUntil:fullClient.profileValidUntil||null,sourceId:fullClient.profileSourceRef||fullClient.profileSource||null},signals:store.val.signals.filter(item=>exactScope(item,tenantId,ownerId)&&(!clientId||item.clientExternalKey===clientId)).slice(-20),learning:{...this.fallbackLearning(clientId,ownerId,tenantId),visitOutcomes:store.val.outcomes.filter(item=>exactScope(item,tenantId,ownerId)&&String(item.client_id??item.clientId)===String(clientId))},memories:memoryHistory.filter(item=>['proposed','verified'].includes(item.status)&&(!item.valid_until||new Date(item.valid_until)>new Date())),memoryHistory,businessHistory,visits:(store.visits||[]).filter(item=>exactScope(item,tenantId,ownerId)&&String(item.clientId??item.client_id)===String(clientId)),interactions:store.interactions.filter(item=>exactScope(item,tenantId,ownerId)&&String(item.clientId??item.client_id)===String(clientId)),commitments,opportunities:(store.opportunities||[]).filter(item=>exactScope(item,tenantId,ownerId)&&String(item.clientId??item.client_id)===String(clientId)),properties:[],fieldReports:[],soilAnalyses:[],ndviObservations:[],manualRecords:[],attachments,priorRecommendations:[]}
    ...
    try{
      const result=await this.db.query(`SELECT c.*,p.id profile_id,p.tenant_id profile_tenant_id,p.client_id profile_client_id,p.primary_profile,p.secondary_profile,p.irt_score,p.nps_score,p.answers,p.evidence profile_evidence,p.source_survey_id,p.valid_until profile_valid_until,p.assessed_at profile_assessed_at,
        ...
        COALESCE((SELECT jsonb_agg(v ORDER BY COALESCE(v.updated_at,v.created_at) DESC) FROM (SELECT id,scheduled_at,objective,process_agreement,summary,next_commitment,next_action_at,status,lifecycle_status,occurred_at,completed_at,cancelled_at,created_at,updated_at FROM visits WHERE tenant_id=$1 AND client_id=c.id ORDER BY COALESCE(updated_at,created_at) DESC LIMIT 30) v),'[]'::jsonb) visits,
  ...
  // `deduplicated` diz que o registro devolvido JA EXISTIA. Sem esse sinal a rota respondia 201
  // Created sem ter criado nada e a galeria reescrevia por cima a identificacao de uma evidencia ja
  // confirmada, sem nova confirmacao e sem historico.
  ...
  async listAttachments({tenantId=this.tenantId,ownerId,clientId,limit=20,offset=0,mimePrefix='',signal,timeoutMs}){
    throwIfPersistenceCancelled(signal)
    tenantId=assertTenantScope(this.tenantId,tenantId)
    const normalizedClientId=clientId?String(clientId):null
    const prefix=String(mimePrefix||'')
    // Sem offset, o 201o arquivo mais recente era inalcancavel por qualquer combinacao de parametros:
    // a foto de agosto da lavoura deixava de existir para o consultor, sem aviso e sem "ver mais".
    // A funcao segue devolvendo um ARRAY (o motor e as telas dependem disso); total/offset/limit vao
    // como propriedades desse array para a rota poder paginar e dizer o tamanho real do acervo.
    const page=Math.max(0,Math.round(Number(offset)||0))
    const size=Math.max(1,Math.min(200,Number(limit)||20))
    if(!this.db.configured){const all=this.fallback().val.attachments.filter(item=>attachmentInTenant(item,tenantId)&&item.ownerId===ownerId&&(item.clientId||null)===(normalizedClientId||null)&&item.status!=='rejected'&&(!prefix||String(item.mimeType||item.mime_type||'').startsWith(prefix))).reverse();const result=Object.assign(all.slice(page,page+size).map(attachmentMetadataRecord),{total:all.length,offset:page,limit:size});throwIfPersistenceCancelled(signal);return result}
    try{const result=await this.db.query("SELECT a.*,NULL::text content_base64,c.external_key client_external_key,COUNT(*) OVER () total_count FROM val_attachments a LEFT JOIN clients c ON c.id=a.client_id AND c.tenant_id=a.tenant_id WHERE a.tenant_id=$1 AND a.consultant_id=$2 AND (($3::text='' AND a.client_id IS NULL) OR ($3::text<>'' AND (c.id::text=$3 OR c.external_key=$3))) AND a.status<>'rejected' AND ($5::text='' OR a.mime_type LIKE $5||'%') ORDER BY a.created_at DESC,a.id DESC LIMIT $4 OFFSET $6",[tenantId,ownerId,normalizedClientId||'',size,prefix,page],{signal,timeoutMs});throwIfPersistenceCancelled(signal);return Object.assign(result.rows.map(attachmentRecord),{total:Number(result.rows[0]?.total_count??0),offset:page,limit:size})}catch(error){if(signal?.aborted)throw persistenceCancellationError(signal);throw serviceError('Os arquivos deste escopo não puderam ser lidos.')}
  }
    ...
    if(this.db.configured){
      try{
        return await this.db.transaction(async client=>{
          ...
          // A chave do ERP e do consultor, nao do tenant: sem owner_user_id no ON CONFLICT, a venda do
          // segundo consultor com o mesmo numero de pedido era engolida pelo DO NOTHING e o webhook
          // ainda respondia 202 accepted:true. RETURNING id diz se a linha entrou mesmo.
  ...
  async ingestCommercialImport({tenantId=this.tenantId,ownerId,summary,clients,rows=[],mapping={}}){
    ...
    try{
      await this.db.transaction(async connection=>{
        ...
        const archivedRows=await connection.query(`SELECT external_key FROM clients WHERE tenant_id=$1 AND consultant_id=$2 AND status='archived' AND external_key=ANY($3::text[])`,[tenantId,ownerId,lockKeys])
