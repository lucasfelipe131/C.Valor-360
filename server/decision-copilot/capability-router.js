...
export function classifyStructuredClientFact(message=''){
 ...
 if(/^(?:e\s+)?(?:como\s+(?:ele|ela|o\s+produtor|a\s+produtora)\s+(?:decide|compra|pensa|escolhe|toma\s+(?:as\s+)?decis(?:ao|oes))|(?:qual|como)\s+(?:e\s+)?(?:o\s+)?(?:estilo|jeito)(?:\s+de\s+(?:decisao|decidir|compra|comprar))?\s+(?:dele|dela))$/.test(source))return 'BEHAVIORAL_PROFILE'
...
function fastProfilePresentation(facts={},now=new Date(),scope={}){
 ...
 const inferenceFact={
  ...
  // A frase "Como abordar" da resposta e derivada do perfil registrado, nao das declaracoes do
  // questionario: sem ela na inferencia, o grounding so aceitava o perfil de quem, por coincidencia,
  // tinha as mesmas palavras nas evidencias (Relacional/Conservador caiam em 400).
  statement:`Leitura comportamental de ${name}: ${primary}${registeredSecondary?` / ${secondary}`:''}. Abordagem derivada do perfil registrado: ${profileApproach(primary)}.`,observed_at:profileEvidenceTimestamp(registeredPrimary)||assessedAt,valid_until:validUntil,confidence:confidence==='alta'?.9:.75
 }
