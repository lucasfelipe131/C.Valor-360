...
function scoreItem(item,{searchTokens,queryConcepts,requestedModules,requestedGeography,sourceById,now}){
 const reasonCodes=[]
...
export function selectKnowledge({query='',contextSnapshot=null,modules=[],geography='General',limit=3,now=new Date(),library=null,libraryOptions=null}={}){
 ...
 for(const item of source.items){
  const result=scoreItem(item,{searchTokens,queryConcepts,requestedModules,requestedGeography:geography,sourceById,now})
  if(!result.eligible){
