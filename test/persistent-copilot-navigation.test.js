...
const keep=new Set(['GlobalValCopilot','ValRealtimeConversation'])
...
async function mountApp({mobile=false}={}){
 ...
 replace('fetch',async(path,options={})=>{
  ...
  if(path==='/api/intelligence')return json({clients,visits:[],opportunities:[]})
  if(path==='/api/auth/logout'||path==='/api/usage/events')return json({ok:true})
  ...
  if(path==='https://voice.mock/calls')return new Response('v=0\n',{status:200})
