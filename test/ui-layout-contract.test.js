...
test('brand mark exposes the VAL identity and reuses the real logo on mobile',()=>{
 ...
 assert.match(logo,/VAL — inteligência que gera valor/)
 // A marca e o ativo oficial, nao um desenho: img em vez de svg/useId.
...
test('manual history and professional profile restore from the account cloud',()=>{
 ...
 assert.match(recordsRoute,/DELETE FROM app_records WHERE id = \$1 AND tenant_id = \$2 AND workspace_id = \$3/)
...
test('inteligência agronômica organiza capacidades nativas e abre o núcleo técnico na mesma sessão',()=>{
 ...
 assert.match(agro,/\/tecnico\?embedded=1&page=/)
 assert.match(agro,/Mantendo sua sessão/)
 assert.match(agro,/Use a ferramenta ou fale com a VAL\./)
 assert.match(agro,/data-agro-hero-action="voice"/)
 assert.match(agro,/data-agro-hero-action="text"/)
 assert.match(app,/<Agro key=\{agroLaunch\.nonce\|\|'agro'\} onAsk=\{openCopilot\} onCapture=\{openCopilot\}/)
