...
test('superfícies HTTP e UI conectam resolver, sessão, voz, métricas e confirmação',()=>{
 ...
 assert.match(server,/sessionState:conversationStateContext\(sessionState\)/)
 assert.match(server,/activeContext:null\}\n  let sessionState=/)
 assert.match(server,/activeContextRef=validateActiveContext[\s\S]*valConversationSessions\.advance\(\{\.\.\.sessionScope,activeContext:activeContextRef\}/)
