...
test('ValCore encaminha ConversationState normalizado à engine existente, sem executor paralelo',async()=>{
 ...
 await core.execute(request,{engineInput:{tenantId:'tenant-a',ownerId:'owner-a',clientId:'client-a',client:{id:'client-a'},intent:'PREPARE_VISIT',conversationId:'thread-a',contextEpoch:0,contextDomain:'VISIT',sessionState:state}})
 assert.equal(received.contextRequest.intent,'PREPARE_VISIT')
 assert.equal(received.contextRequest.conversationId,'thread-a')
 assert.equal(received.contextRequest.contextEpoch,0)
 assert.equal(received.contextRequest.contextDomain,'VISIT')
...
test('superfícies HTTP e UI conectam resolver, sessão, voz, métricas e confirmação',()=>{
 ...
 assert.match(copilot,/realtimeContext=\{\{clientId:client\?\.id\|\|'',conversationId:realtimeConversationId,contextEpoch:realtimeContextEpoch,activeContext\}\}/)
 assert.match(copilot,/serverGrounded:true,grounding:'SERVER_RETURNED'/)
 assert.match(copilot,/responseScope=assertResponseScopeForRequest\(payload,/)
 assert.match(copilot,/realtimeTurnMatchesScope\(callbackScope,realtimeScope\)/)
 assert.match(copilot,/status:'incomplete',serverGrounded:false,grounding:'UNVERIFIED_BROWSER_TRANSCRIPT'/)
