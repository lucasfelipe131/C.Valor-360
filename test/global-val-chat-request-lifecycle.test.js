...
test('nova pergunta isola o turno pendente e troca de escopo cancela a anterior sem stale writes',()=>{
 ...
 assert.match(switches,/const chooseClient=id=>\{[\s\S]*?if\(uploading\)return[\s\S]*?cancelChatRun\(\);cancelRealtimeClarification\(\);/)
 assert.match(switches,/const newConversation=.*if\(uploading\)return\s*cancelChatRun\(\);cancelRealtimeClarification\(\)/s)
 assert.match(switches,/const selectHistory=item=>\{if\(uploading\)return;[\s\S]*?cancelChatRun\(\);cancelRealtimeClarification\(\);/)
