...
test('entradas em linguagem corrente: espaçamento em metros, percentual antes da palavra-chave, unidade da dose, desconto em reais, ZARC e produtividade',async()=>{
 const planterMeters=parseAgronomicCalculatorRequest('regulagem da semeadora: população 70.000, espaçamento 0,45 m, germinação 95%, sobrevivência 92%, 2% de patinagem, embalagem 60.000 sementes','semeadora')
 assert.equal(planterMeters.spacingCm,45)
 assert.equal(planterMeters.slippagePercent,2)
 assert.equal(parseAgronomicCalculatorRequest('população ideal de soja cultivar X com espaçamento de 45 cm','populacao').spacingCm,45)
 const arithmetic=await executeCopilotCalculator('quantas plantas por metro para 300 mil plantas/ha com espaçamento de 0,45 m?')
 assert.equal(arithmetic.input.spacing_cm,45)
 assert.equal(arithmetic.output.plants_per_meter,13.5)
 const spraying=parseAgronomicCalculatorRequest('pulverização: área 120 ha, volume de calda 100 L/ha, volume do tanque 2000 L, produto Inseticida X, dose 500 mL/ha','pulverizacao')
 assert.deepEqual(spraying.items,[{product:'inseticida x',dose:500,unit:'mL/ha'}])
 assert.equal(parseAgronomicCalculatorRequest('pulverização: área 80 ha, volume de calda 150 L/ha, tanque 3000 L, produto Fungicida Y, dose 1,2 kg/ha','pulverizacao').items[0].unit,'kg/ha')
 assert.equal(parseAgronomicCalculatorRequest('fertilizante 04-14-08, área 120 ha, dose 300 kg/ha, saco 50 kg, preço R$ 3 por kg, com 90% de eficiência','fertilizante').efficiencyPercent,90)
 const quoteBefore=parseAgronomicCalculatorRequest('cotação de insumos: produto Fertilizante, quantidade 100 sacos, preço de sistema R$ 165, com 5% de desconto.','cotacao')
 assert.equal(quoteBefore.items[0].discountPercent,5)
 const quoteReais=parseAgronomicCalculatorRequest('cotação de insumos: produto Fertilizante, quantidade 100 sacos, preço de sistema R$ 165, desconto de R$ 10.','cotacao')
 assert.ok(Math.abs(quoteReais.items[0].discountPercent-6.0606)<0.001)
 assert.equal(identifyAgronomicCalculator('cotação de insumos: produto Semente de soja, quantidade 100 sacos, preço de sistema R$ 500, desconto 5%.'),'cotacao')
 assert.equal(identifyAgronomicCalculator('quantas sementes preciso para 100 ha com população 300 mil e embalagem de 60 mil?'),'sementes')
 const zarc=parseAgronomicCalculatorRequest('ZARC para soja no município Dourados, UF MS, plantio dia 15/11, solo AD3, grupo 2.','zoneamento')
 assert.equal(zarc.soil,'13')
 assert.equal(zarc.cycle,'21')
 assert.equal(parseAgronomicCalculatorRequest('ZARC para soja em Dourados/MS, 12 linhas, solo 3, grupo II','zoneamento').soil,'3')
 for(const phrase of ['produtividade esperada de 70 sc/ha','produtividade de 70 sacas por hectare','meta 70 sc/ha']){
  const parsed=parseAgronomicCalculatorRequest(`calcula reposição de nutrientes para soja com ${phrase} pela exportação`,'reposicao')
  assert.equal(parsed.yieldValue,70,phrase);assert.equal(parsed.yieldUnit,'sc/ha',phrase)
 }
 const removal=await executeCopilotCalculator('calcula reposição de nutrientes para soja com produtividade 70 sc/ha pela exportação')
 assert.match(removal.summary,/necessidade em kg\/ha: N \d/)
})
