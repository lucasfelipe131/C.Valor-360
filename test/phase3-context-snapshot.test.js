...
const baseContext=()=>({client:{id:'client-1',name:'Produtor Teste'},profile:{evidence:[{id:'survey-1',source:'producer_360'}]},businessHistory:[],visits:[],interactions:[],opportunities:[],properties:[],fieldReports:[],soilAnalyses:[],ndviObservations:[],memories:[],memoryHistory:[]})
const memory=(id,overrides={})=>({id,tenant_id:tenantA,client_id:'client-1',subject_type:'client',subject_id:'client-1',memory_type:'fact',memory_state:'FACT',memory_domain:'PRODUCER',key:'planted_area_ha',value:500,status:'verified',source:'consultant_input',source_ref:`consultant_input:${id}`,source_type:'consultant_input',confidence:80,valid_from:'2026-01-01T12:00:00.000Z',created_at:'2026-01-01T12:00:00.000Z',updated_at:'2026-01-01T12:00:00.000Z',acl:{scope:'own_portfolio'},...overrides})
...
test('análise de solo antiga é recuperada apenas com stale flag explícita',()=>{
  const context=baseContext()
  context.soilAnalyses=[{id:'soil-old',sampled_at:'2020-02-01',laboratory:'Lab A',measurements:[{analyte:'pH',normalized_value:5.2}]}]
...
test('análise de solo sem data permanece freshness desconhecida e não fecha lacuna',()=>{
  const context=baseContext()
  context.soilAnalyses=[{id:'soil-undated',laboratory:'Lab sem data'}]
  const result=snapshot(context,{objective:'agronomic_question'})
...
test('tenant adversarial é descartado antes de refs, fatos e conflitos',()=>{
  ...
  assert.equal(JSON.stringify(result).includes('memory-foreign'),false)
...
test('memória de propriedade e organização exige vínculo no contexto autorizado',()=>{
  const context=baseContext()
  context.properties=[{id:'property-1',fields:[{id:'field-1'}]}]
  context.memoryHistory=[
    ...
    memory('memory-organization',{subject_type:'organization',subject_id:tenantA,memory_domain:'ORGANIZATIONAL',key:'commercial_policy',acl:{scope:'organization',roles:['consultant']}}),
    memory('memory-organization-unreviewed',{subject_type:'organization',subject_id:tenantA,memory_domain:'ORGANIZATIONAL',key:'legacy_policy',source_ref:'organization:unreviewed'}),
    memory('memory-unrelated-property',{subject_type:'property',subject_id:'property-foreign',source_ref:'property:foreign'})
  ]
  const result=snapshot(context)
  assert.deepEqual(new Set(result.selection.selected_refs),new Set(['memory-property','memory-field','memory-organization']))
  assert.equal(result.selection.unauthorized_count,2)
  assert.equal(JSON.stringify(result).includes('property:foreign'),false)
  assert.equal(JSON.stringify(result).includes('organization:unreviewed'),false)
})
...
test('dez anos de histórico retornam o conjunto relevante, não o histórico inteiro',()=>{
  ...
  assert.equal(result.selection.selected_refs.length,24)
  ...
  assert.equal(result.selection.exclusion_reason_codes.filter(item=>item.reason_codes.includes('LOWER_RELEVANCE')).length,96)
...
test('perfil comportamental é sinal observável, nunca fato absoluto',()=>{
  ...
  context.client.profileEvidence=[{id:'answer-1',source:'producer_360'}]
  const result=snapshot(context)
...
test('freshness é avaliada por domínio e fonte, sem TTL universal',()=>{
  const context=baseContext()
  context.businessHistory=[{id:'business-old',occurred_at:'2010-01-01T12:00:00.000Z'}]
  context.soilAnalyses=[{id:'soil-old',sampled_at:'2020-01-01T12:00:00.000Z'}]
  const result=snapshot(context,{objective:'agronomic_question'})
  assert.equal(result.commercial_context.business_history[0].freshness,'UNKNOWN')
  assert.equal(result.commercial_context.business_history[0].freshness_metadata.reason_code,'NO_DOMAIN_SOURCE_POLICY')
  assert.equal(result.agronomic_context.soil_analyses[0].freshness,'STALE')
