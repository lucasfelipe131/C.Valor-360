...
test('dossiê do produtor oferece galeria de lavoura com metadados e armazenamento protegido',()=>{
 ...
 assert.match(gallery,/fieldPhoto:metadata/)
 assert.match(gallery,/attachmentContentUrl\(item,\{clientId\}\)/)
 assert.match(gallery,/\?clientId=\$\{encodeURIComponent\(clientId\)\}/)
 assert.doesNotMatch(gallery,/analysis:\{\.\.\.\(item\.analysis/)
