...
test('VoiceCapture frontend — lifecycle canônico bloqueia superfícies em estados terminais',()=>{
 ...
 assert.match(visits,/postVisitVoiceLifecycle=new Set\(\['PLANNED','PREPARED','IN_PROGRESS','COMPLETED_PENDING_REVIEW'\]\)/)
 assert.doesNotMatch(visits,/postVisitVoiceLifecycle=new Set\(\[[^\]]*'(?:COMPLETED|CANCELLED)'[^\]]*\]\)/)
